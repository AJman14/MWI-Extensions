...
var ngicGame = "[class*='GamePage_gamePage__']";// Add classes to this for disabled skills
...
var ngicPanel = "[class*='GatheringProductionSkillPanel_gatheringProductionSkillPanel'], [class*='TasksPanel_tasksPanel'], [class*='AlchemyPanel_alchemyPanel'], [class*='EnhancingPanel_enhancingPanel']";
var ngicPanelTitle = "[class*='GatheringProductionSkillPanel_title'] > div, [class*='TasksPanel_title'], [class*='AlchemyPanel_title'] > div, [class*='EnhancingPanel_title'] > div";
var character = '';
...
document.head.insertAdjacentHTML("beforeend", `<style type="text/css">
    ...
    /* Disabled Skill panel */
    .ngicDisabledPanel{
        position: relative;
        pointer-events: none !important;
    }
    .ngicDisabledPanel:after {
        content: '';
        position: absolute;
        inset: 0;
        background: rgba(var(--color-background-game-rgb), 0.85);
        z-index: 1;
        font-size: 1.5rem;
        font-weight: bold;
        text-shadow: 0px 0px 1em var(--color-background-game), 0px 0px 1em var(--color-background-game), 0px 0px 1em var(--color-background-game);
    }
    .ngicDisabledPanel :is(`+ngicPanelTitle+`){
        opacity: 0;
    }
    .ngicDisabledPanel.ngicDisabledPanel-ngic:after{
        content: 'No Gathering';
    }
    .ngicDisabledPanel.ngicDisabledPanel-ntic:after{
        content: 'No Tasks';
    }
    .ngicDisabledPanel.ngicDisabledPanel-coic:after{
        content: 'Combat Only';
    }
    .ngicDisabledPanel.ngicDisabledPanel-cone:after{
        content: 'No Enhancing';
    }
    .ngicDisabledPanel [class*='Modal_modalContainer']{
        display: none;
    }
</style>`)
...
const ngicObserver = new MutationObserver(mutations => {
    /* Add options to the Settings page */
    ...
    /* If the header exists, init all 4 options */
    ...
    /* If the skill is disabled, disable the panel */
    toggleSkillPanel()
});
...
function toggleSkillPanel(){
    /* "No Gathering" overlay if you are in a disabled skill (Milking is selected when you open the game if your character was idle...) */
    if (document.querySelector(ngicPanel)) {
        let title = document.querySelector(ngicPanelTitle);
        //console.log(title.innerText+" panel detected. Adding overlay if category is disabled.");
        if (localStorage.getItem(character+'-ngicToggle') == 'checked'){
            if (title.innerText == 'Milking' || title.innerText == 'Foraging' || title.innerText == 'Woodcutting'){
                document.querySelector(ngicPanel).classList.add('ngicDisabledPanel');
                document.querySelector(ngicPanel).classList.add('ngicDisabledPanel-ngic');
            } else if (localStorage.getItem(character+'-nticToggle') == 'checked' && title.innerText == 'Tasks'){
                document.querySelector(ngicPanel).classList.add('ngicDisabledPanel');
                document.querySelector(ngicPanel).classList.add('ngicDisabledPanel-ntic');
            } else if (localStorage.getItem(character+'-coicToggle') == 'checked'){
                if (title.innerText == 'Cheesesmithing' || title.innerText == 'Crafting' || title.innerText == 'Tailoring' || title.innerText == 'Cooking' || title.innerText == 'Brewing' || title.innerText == 'Alchemy'){
                    document.querySelector(ngicPanel).classList.add('ngicDisabledPanel');
                    document.querySelector(ngicPanel).classList.add('ngicDisabledPanel-coic');
                } else if (localStorage.getItem(character+'-coneToggle') == 'checked' && title.innerText == 'Enhancing'){
                    document.querySelector(ngicPanel).classList.add('ngicDisabledPanel');
                    document.querySelector(ngicPanel).classList.add('ngicDisabledPanel-cone');
                }
            }
        } else {
            document.querySelector(ngicPanel).classList.remove('ngicDisabledPanel');
        }
    }
}
